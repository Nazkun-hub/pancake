...
app.get('/api/config/status', async (req, res) => {
  try {
    const hasConfig = dexClient !== null && dexClient !== undefined;
    let isConnected = false;
    
    if (hasConfig) {
      try {
        isConnected = await dexClient.checkConnection();
      } catch (error) {
    ...
    res.status(500).json({
      success: false,
      message: error instanceof Error ? error.message : '检查配置状态失败'
    });
  }
});
...
app.post('/api/swap', async (req, res) => {
  try {
    ...
    console.log(`[${timeStr}] 🚀 执行BSC交换请求:`, JSON.stringify(req.body, null, 2));
    ...
    console.log(`[${getLocalTimeString()}] ✅ BSC交换结果:`, JSON.stringify(result, null, 2));
    ...
    console.error(`[${getLocalTimeString()}] ❌ 执行BSC交换失败:`, error);
...
// 启动服务器
app.listen(port, () => {
  console.log('✅ OKX DEX API服务启动成功!');
  console.log('=====================================================');
  console.log('🎮 OKX DEX API 交易服务');
  console.log('=====================================================');
  console.log(`🌐 OKX DEX界面: http://localhost:${port}`);
  console.log(`📡 API服务: http://localhost:${port}/api`);
  console.log(`🔧 调试页面: http://localhost:${port}/debug`);
  console.log(`🔗 专注BSC链: Chain ID 56`);
  console.log('');
  console.log('🌐 相关系统服务:');
